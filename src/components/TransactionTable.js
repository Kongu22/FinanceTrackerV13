...
// Register the custom plugin for center text
...
const TransactionTable = ({ transactions, addTransaction, editTransaction, deleteTransaction }) => {
  ...
  const expensePieOptions = {
    ...
    plugins: {
      ...pieOptions.plugins,
      centerText: {
        text: `₪${formatAmountForChart(totalExpenses)}`, // Shortened format
        color: 'red',
        font: {
          size: isMobile ? 20 : 20,
          weight: 'bold',
  ...
  const incomePieOptions = {
    ...
    plugins: {
      ...pieOptions.plugins,
      centerText: {
        text: `₪${formatAmountForChart(totalIncome)}`, // Shortened format
        color: 'green',
        font: {
          size: isMobile ? 20 : 20,
          weight: 'bold',
    ...
    <>
      ...
      <Box
        sx={{
          ...
          mt: 8,
          position: 'relative',
          gap: isMobile ? 0 : 6, // Reduce the gap between pie charts on small screens
        }}
      >
        {/* Expense Pie Chart */}
        <Box
        ...
        <Box
          sx={{
            width: '2px',
            height: isMobile ? 180 : 230, // Adjust height based on the screen size
            backgroundColor: 'gray',
            mx: 2, // Add margin between the charts
          }}
        />
